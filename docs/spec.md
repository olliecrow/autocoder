...
- **stop signal**: autocoder must only act on issues that currently have the opt-in label `autocoder`; if `autocoder` is removed, autocoder must stop working on that issue.
